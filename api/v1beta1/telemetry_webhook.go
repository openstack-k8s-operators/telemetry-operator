...
type TelemetryDefaults struct {
	...
	SgCoreImageURL       string
}

var (
	telemetryDefaults TelemetryDefaults
	telemetrylog      = logf.Log.WithName("telemetry-resource")
)

// SetupTelemetryDefaults set's default values for Telemetry resources globaly
...
// Default - set defaults for this TelemetrySpec
func (spec *TelemetrySpec) Default() {
	if spec.CeilometerCentral.InitImage == "" {
		spec.CeilometerCentral.InitImage = telemetryDefaults.InitImageURL
	}

	if spec.CeilometerCentral.CentralImage == "" {
		spec.CeilometerCentral.CentralImage = telemetryDefaults.CentralImageURL
	}

	if spec.CeilometerCentral.NotificationImage == "" {
		spec.CeilometerCentral.NotificationImage = telemetryDefaults.NotificationImageURL
	}

	if spec.CeilometerCentral.SgCoreImage == "" {
		spec.CeilometerCentral.SgCoreImage = telemetryDefaults.SgCoreImageURL
	}
}
