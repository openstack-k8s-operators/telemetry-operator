...
func ScrapeConfigRabbitMQ(
	...
	// NOTE: we use the Endpoints object to retrieve a list of all pods
	...
	err := helper.GetClient().Get(ctx, namespacedName, endpoints)
	if err != nil {
		return nil, err
	}
	...
	for _, subset := range endpoints.Subsets {
		for _, address := range subset.Addresses {
			target := ""
			if tlsEnabled {
				target = net.JoinHostPort(address.IP, RabbitMQPrometheusPortTLS)
			} else {
				target = net.JoinHostPort(address.IP, RabbitMQPrometheusPortNoTLS)
			}
			staticConfigs = append(staticConfigs, monv1alpha1.StaticConfig{
				Targets: []monv1alpha1.Target{monv1alpha1.Target(target)},
				Labels: map[monv1.LabelName]string{
					monv1.LabelName("pod"): address.TargetRef.Name,
				},
			})
		}
	}
