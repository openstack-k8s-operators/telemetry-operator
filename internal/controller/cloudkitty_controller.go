...
func (r *CloudKittyReconciler) reconcileDelete(ctx context.Context, instance *telemetryv1.CloudKitty, helper *helper.Helper) (ctrl.Result, error) {
	...
	for secretName := range transportSecrets {
		if err := object.RemoveSecretConsumerFinalizer(ctx, helper, instance.Namespace,
			secretName, cloudkitty.TransportConsumerFinalizer); err != nil {
			return ctrl.Result{}, err
		}
	}
	...
	if err := object.PruneSecretConsumerFinalizers(
		ctx, helper, instance.Namespace, cloudkitty.TransportConsumerFinalizer,
	); err != nil {
		return ctrl.Result{}, err
	}
	if err := object.PruneSecretConsumerFinalizers(
		ctx, helper, instance.Namespace, cloudkitty.ACConsumerFinalizer,
	); err != nil {
		return ctrl.Result{}, err
	}
...
func (r *CloudKittyReconciler) reconcileNormal(ctx context.Context, instance *telemetryv1.CloudKitty, helper *helper.Helper) (ctrl.Result, error) {
	...
	if err := object.ManageSecretConsumerFinalizer(ctx, helper, instance.Namespace,
		currentTransportSecret, cloudkitty.TransportConsumerFinalizer); err != nil {
		return ctrl.Result{}, err
	}
	...
	//
	_, _, err = r.createHashOfInputHashes(ctx, instance, configVars)
	if err != nil {
		instance.Status.Conditions.Set(condition.FalseCondition(
			condition.ServiceConfigReadyCondition,
			condition.ErrorReason,
	...
	expectedInputHash, err := util.ObjectHash(rotatingSecrets)
	if err != nil {
		return ctrl.Result{}, err
	}
	...
	secretName, err := object.FinalizeSecretRotation(
		...
		cloudkitty.TransportConsumerFinalizer,
		allServicesReady,
	)
	if err != nil {
		return ctrl.Result{}, err
