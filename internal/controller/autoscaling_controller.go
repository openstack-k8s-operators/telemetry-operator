...
func (r *AutoscalingReconciler) reconcileDelete(
	...
	for secretName := range transportSecrets {
		if err := object.RemoveSecretConsumerFinalizer(ctx, helper, instance.Namespace,
			secretName, autoscaling.TransportConsumerFinalizer); err != nil {
			return ctrl.Result{}, err
		}
	}
	...
	if err := object.PruneSecretConsumerFinalizers(
		ctx, helper, instance.Namespace, autoscaling.TransportConsumerFinalizer,
	); err != nil {
		return ctrl.Result{}, err
	}
	if err := object.PruneSecretConsumerFinalizers(
		ctx, helper, instance.Namespace, autoscaling.ACConsumerFinalizer,
	); err != nil {
		return ctrl.Result{}, err
	}
...
func (r *AutoscalingReconciler) reconcileNormal(
	...
	if err := object.ManageSecretConsumerFinalizer(ctx, helper, instance.Namespace,
		currentNotifSecret, autoscaling.TransportConsumerFinalizer); err != nil {
		return ctrl.Result{}, err
	}
	...
	secretName, err := object.FinalizeSecretRotation(
		...
		oldNotifSecret,
		currentNotifSecret,
		autoscaling.TransportConsumerFinalizer,
		guardReady,
	)
	if err != nil {
		return ctrl.Result{}, err
