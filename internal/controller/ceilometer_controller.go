...
func (r *CeilometerReconciler) reconcileDelete(ctx context.Context, instance *telemetryv1.Ceilometer, helper *helper.Helper) (ctrl.Result, error) {
	...
	for secretName := range transportSecrets {
		if err := object.RemoveSecretConsumerFinalizer(ctx, helper, instance.Namespace,
			secretName, ceilometer.TransportConsumerFinalizer); err != nil {
			return ctrl.Result{}, err
		}
	}
	...
	if err := object.PruneSecretConsumerFinalizers(
		ctx, helper, instance.Namespace, ceilometer.TransportConsumerFinalizer,
	); err != nil {
		return ctrl.Result{}, err
	}
	if err := object.PruneSecretConsumerFinalizers(
		ctx, helper, instance.Namespace, ceilometer.ACConsumerFinalizer,
	); err != nil {
		return ctrl.Result{}, err
	}
...
func (r *CeilometerReconciler) reconcileCeilometer(
	...
	if err := object.ManageSecretConsumerFinalizer(ctx, helper, instance.Namespace,
		currentNotifSecret, ceilometer.TransportConsumerFinalizer); err != nil {
		return ctrl.Result{}, err
	}
	...
	err = r.generateServiceConfig(ctx, helper, instance, &configMapVars, notificationBusInstanceURL.Status.SecretName, telemetrySecret)
	if err != nil {
		instance.Status.Conditions.Set(condition.FalseCondition(
			condition.ServiceConfigReadyCondition,
			condition.ErrorReason,
	...
	// - parameters which has passwords gets added from the OpenStack secret via the init container
	//
	err = r.generateComputeServiceConfig(ctx, helper, instance, &configMapVars, notificationBusInstanceURL.Status.SecretName, telemetrySecret)
	if err != nil {
		instance.Status.Conditions.Set(condition.FalseCondition(
			condition.ServiceConfigReadyCondition,
			condition.ErrorReason,
	...
	secretName, err := object.FinalizeSecretRotation(
		...
		oldNotifSecret,
		currentNotifSecret,
		ceilometer.TransportConsumerFinalizer,
		guardReady,
	)
	if err != nil {
		return ctrl.Result{}, err
